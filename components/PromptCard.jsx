...
const PromptCard = ({ post, handleTaglick, handleEdit, handleDelete }) => {
  ...
  return (
    <div className="prompt_card">
      ...
      <p className="font-inter text-sm blue_gradient cursor-pointer" onClick={() => handleTaglick && handleTaglick(post.tag)}>{post.tag}</p>
      {session?.user.id === post.creator._id && pathName === '/profile' && (
        <div className="mt-5 flex-center gap-4 border-t border-gray-200 pt-3">
          ...
          <p className="font-inter text-sm red_gradient cursor-pointer" onClick={handleDelete}>Delete</p>
      </div>
      )}
    </div>
  );
